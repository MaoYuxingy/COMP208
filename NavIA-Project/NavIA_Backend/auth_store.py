...
def _to_user_record(user: models.DBUser) -> UserRecord:
    return {
        ...
        "password_salt": user.password_salt or "pbkdf2_managed",
...
def _build_demo_users() -> Dict[str, UserRecord]:
    demo_email = os.getenv("AUTH_DEMO_EMAIL", "test@navia.app")
    demo_password = os.getenv("AUTH_DEMO_PASSWORD", "NavIA123!")
...
def ensure_demo_user_in_db():
    if not _DEMO_USERS:
        return

    demo_user = next(iter(_DEMO_USERS.values()))

    db = SessionLocal()
    try:
        user = db.query(models.DBUser).filter(models.DBUser.email == demo_user["email"]).first()
        ...
        if user is None:
            db.add(
                models.DBUser(
                    ...
                    password_salt=demo_user["password_salt"],
            ...
            user.password_salt = demo_user["password_salt"]
